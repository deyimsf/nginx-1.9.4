...
ngx_mail_block(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
     ...
     *         -----------------------
     *
     * 我们这里其实是要判断结构体ngx_mail_conf_ctx_t是否被创建过，如果被创建过，
     * 那么第二层指针中存放的就是这个结构体的地址，否则它的值为空。
     *
     *
     * 为什么要先转成两星指针，我们这里只是判断第二层指针是否有值，直接在conf前加个星不就可以吗？
     * 答案肯定是不行，因为编译器无法判断一个void类型的指针变量指向的是什么类型的数据。
     *
     * 如果我们的conf是这样定义的:
     *    int *conf;
     * 编译器会认为它的结构是这样的:
     ...
     *      | int |
     *      -------
     * 这是一个正确的结构，所以使用*conf的时候自然也是可以获取到值的，直接从开始地址读取sizeof(int)个字节，并把读到的字节当成int解释就可以了。
     *
     * 但如果conf是这样定义的:
     *    void *conf
     * 它的结构应该是这样:
     ...
     *       | void ? |
     *       ----------
     * 此时用*conf获取值的时候，编译器无法判断该值的类型，也就无法确定应该从开始地址读取多少个字节。
     *
     *
     * 转成两层指针后再使用*conf取值，则编译器会明确知道，要取的值就是一个void*类型的指针，所以从开始地址读取sizeof(void *)字节就可以了。
     *
     */

    if (*(ngx_mail_conf_ctx_t **) conf) {
    	/**
		 * 如果能走到这里，说明配置文件中有重复的events{}块了
		 *
		 *          conf
		 *         -----
		 *         | * |
