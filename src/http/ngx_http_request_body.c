    ...
    ngx_http_client_body_handler_pt post_handler)
{
    ...
    /*
     * 先把主请求的引用计数器加一
     * 因为ngx在读取请求体的时候也同时在做别的事情,比如说在读取请求体的过程中已经把响应数据都
     * 发送完给客户端了,正常情况下ngx就可以将这个请求关闭了,但是如果这个关闭了那前面读请求体的
     * 操作怎么办? 所以这里通过一个计数器来标记当前请求还不能结束.
     *
     * 可以把这个计数器理解成一个异步操作个数计数器,也就是说只要这个主请求发起了一个异步操作,这个
     * 计数器就要加一,这样ngx就可以通过这个计数器来获知是否和这个请求关联的未结束的操作
     */
    r->main->count++;
