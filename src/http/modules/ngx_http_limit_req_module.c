    ...
    ngx_str_t *key, ngx_uint_t *ep, ngx_uint_t account)
{
    ...
    while (node != sentinel) {
        ...
        if (rc == 0) {
            ...
            /**
             * 本次遗留的请求数 = 上次遗留的请求数 - 速率 * 过去的时间 + 1(这里的1表示本次请求)
             *
             * ngx内部把时间单位缩小了1000倍，所以最后加1000
             * ngx内部把时间单位缩小了1000倍，所以速率就需要扩大1000倍，原来速率表示每秒1个，现在表示每1000毫秒1000个
             *
             * 除以1000是把毫秒转换成秒
             *
             * lr->excess ===>                     上次遗留的请求数
             * ctx->rate * ngx_abs(ms)/1000 ===>   预测过去的时间里产生的请求数(速率已经扩大了，所以这里的时间就不用再调整，所以这里要把毫秒转成秒)
             * + 1000   ========> 每秒1次，变成毫秒就是每1000毫秒1000次
             */
